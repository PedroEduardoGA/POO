...
public abstract class Principal {
	...
	public static void main(String[] args) {
		...
		do{
			...
			if(opcao==2) {
				...
				if(indice != -1) {
					...
					if (conta instanceof ContaCorrente) {//se a conta pega com aquele numero é uma instancia de conta corrente
			...
			if(opcao==3) {
				...
				if(indice != -1) {
					conta=contas.get(indice);
					if (conta instanceof ContaPoupanca) {//se a conta pega com aquele indice é uma instancia de conta poupanca
						((ContaPoupanca) conta).calcularNovoSaldo(rendimento);			
					}else
						System.out.println("Conta: " +nroConta +(" não é do tipo conta Poupanca!"));
				}else
					System.out.println("Conta inexistente!");
			}
			...
			if(opcao==5) {
				...
				if(indice != -1) {
					...
					System.out.print(" é de R$" +contas.get(indice).saldo);
				}else
					System.out.println("Conta inexistente!");
			}
			...
			if(opcao == 6) {
				...
				if(indice != -1) {
					conta=contas.get(indice);
					if (conta instanceof ContaInvestimento) {//se a conta pega com aquele indice é uma instancia de conta investimento
						((ContaInvestimento) conta).calcularTributo(BigDecimal.valueOf(rendimento));			
					}else
						System.out.println("Conta: " +nroConta +(" não é do tipo conta Investimento!"));
				}else
					System.out.println("Conta inexistente!");
			}
		...
		System.out.println("Atendimento finalizado, o P.O.O Bank agradece sua preferência!");
	}
